...
class MchHttpRequest
{
	...
	public static function getServerRequestTime($withMicroSecondPrecision = false)
	{
		...
		if($withMicroSecondPrecision && isset($_SERVER['REQUEST_TIME_FLOAT'])){
			return sanitize_text_field(wp_unslash($_SERVER['REQUEST_TIME_FLOAT'] ?? ''));
		}

		return $requestTime = empty($_SERVER['REQUEST_TIME']) ? time() : sanitize_text_field(wp_unslash($_SERVER['REQUEST_TIME']));
	}
	...
	public static function getClientIp(array $arrTrustedProxyIps = array())
	{
		...
		(!empty($_SERVER['HTTP_REMOTE_ADDR']) && empty( $_SERVER['REMOTE_ADDR'])) ? $_SERVER['REMOTE_ADDR'] = sanitize_text_field(wp_unslash($_SERVER['HTTP_REMOTE_ADDR'])) : null;

		if(empty($_SERVER['REMOTE_ADDR']) || -1 === ($ipVersion = MchIPUtils::getIpAddressVersion(sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? '')))))
			return null;
		...
		if(!empty($arrTrustedProxyIps) && in_array(sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? '')), $arrTrustedProxyIps, true))
		{
		...
		return $clientIp = sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? ''));
	...
	public static function getClientIpAddressFromProxyHeader($proxyHeader)
	{
		...
		$arrClientIps = explode(',',  sanitize_text_field(wp_unslash($_SERVER[$proxyHeader])));
	...
	private static function getIpAddressFromSucuriCloudProxy()
	{
		...
		if(isset($arrKnownSucuriIps[$_SERVER['REMOTE_ADDR']]))
			return self::$sucuriCloudProxyIp = sanitize_text_field(wp_unslash($_SERVER['HTTP_X_SUCURI_CLIENTIP'] ?? ''));

		$arrKnownSucuriIps = null;

		$hostAddress = null;
		if(!empty($_SERVER['SERVER_ADDR']))
			$hostAddress = sanitize_text_field(wp_unslash($_SERVER['SERVER_ADDR']));
		elseif(!empty($_SERVER['LOCAL_ADDR']))
			$hostAddress = sanitize_text_field(wp_unslash($_SERVER['LOCAL_ADDR']));
		elseif(!empty($_SERVER['SERVER_NAME']))
			$hostAddress = @gethostbyname(sanitize_text_field(wp_unslash($_SERVER['SERVER_NAME'])) . '.');
		...
		return @preg_match('/^cloudproxy[0-9]+\.sucuri\.net$/', $hostName) ? self::$sucuriCloudProxyIp = sanitize_text_field(wp_unslash($_SERVER['HTTP_X_SUCURI_CLIENTIP'] ?? '')) : null;

	}

	private static function getIpAddressFromRackSpace()
	{
		if(0 !== strpos(sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? '')), '10.'))
			return null;
		...
		foreach($arrProxyHeaders as $proxyHeader)
		{
			...
			if( ! MchGdbcTrustedIPRanges::isIPInRackSpaceRanges(sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? '')), MchIPUtils::getIpAddressVersion(sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? '')))) )
				continue;
	...
	{
		if(empty($_SERVER['HTTP_INCAP_CLIENT_IP']) || (-1 === ($ipVersion = MchIPUtils::getIpAddressVersion(sanitize_text_field(wp_unslash($_SERVER['HTTP_INCAP_CLIENT_IP']))))))
			return null;

		return MchGdbcTrustedIPRanges::isIPInIncapsulaRanges( sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? '')), MchIPUtils::getIpAddressVersion(sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? ''))) ) ? sanitize_text_field(wp_unslash($_SERVER['HTTP_INCAP_CLIENT_IP'] ?? '')) : null;
	...
	private static function getIpAddressFromCloudFlare()
	{
		...
		if(empty($_SERVER['HTTP_CF_CONNECTING_IP']) || -1 === ($ipVersion = MchIPUtils::getIpAddressVersion(sanitize_text_field(wp_unslash($_SERVER['HTTP_CF_CONNECTING_IP'])))))
			return $ipAddress = null;

		return $ipAddress = MchGdbcTrustedIPRanges::isIPInCloudFlareRanges( sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? '')), MchIPUtils::getIpAddressVersion(sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? ''))) ) ? sanitize_text_field(wp_unslash($_SERVER['HTTP_CF_CONNECTING_IP'] ?? '')) : null;
	...
	private static function getIpAddressFromAmazonEC2()
	{
		...
		if( ! MchIPUtils::isPublicIpAddress(sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? ''))) && MchIPUtils::isPublicIpAddress($proxyIpAddress) )
		{
			$isAmazon =  ( !empty($_SERVER['SERVER_SOFTWARE'])  && (false !== stripos(sanitize_text_field(wp_unslash($_SERVER['SERVER_SOFTWARE'])), 'Amazon')) ) ? true : ( !empty($_SERVER['SERVER_SIGNATURE'])  && (false !== stripos(sanitize_text_field(wp_unslash($_SERVER['SERVER_SIGNATURE'])), 'Amazon')) );
		...
		return MchGdbcTrustedIPRanges::isIPInAmazonEC2Ranges( sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? '')), MchIPUtils::getIpAddressVersion(sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? ''))) ) ? $proxyIpAddress : null;
	}

	private static function getIpAddressFromAmazonCloudFront()
	{
		if( !empty($_SERVER['HTTP_X_AMZ_CF_ID']) || (!empty($_SERVER['HTTP_USER_AGENT']) && sanitize_text_field(wp_unslash($_SERVER['HTTP_USER_AGENT'])) === 'Amazon CloudFront') || (!empty($_SERVER['HTTP_VIA']) &&  false !== stripos(sanitize_text_field(wp_unslash($_SERVER['HTTP_VIA'])), 'CloudFront')) )
		{

			if(null === ($proxyIpAddress = self::getClientIpAddressFromProxyHeader('HTTP_X_FORWARDED_FOR')))
				return null;

			return MchGdbcTrustedIPRanges::isIPInAmazonCloudFrontRanges( sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? '')), MchIPUtils::getIpAddressVersion(sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? ''))) ) ? $proxyIpAddress : null;
